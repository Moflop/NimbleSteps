...
public class ParkourAnimationManager {
	...
	private static PlayerAnimationController getOrCreateController(AbstractClientPlayer player) {
		...
		// 实例化新版核心类 PlayerAnimationController
	...
	public static void syncStateAnimation(Player player) {
		...
		ParkourAnim targetAnim = currentState.getLinkedAnimation(player);
		...
		if (targetAnim != null) {
			...
			ParkourModifierRegistry.applyModifiers(controller, clientPlayer, currentState, variant);
