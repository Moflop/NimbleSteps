...
public class WallSlideLogic {
	 ...
	 * 执行滑墙时的物理运动与速度修改
	 */
	public static void applySlowdownAndTrySwitchCollisionDirection(Player player, WallData wallData) {
		Vec3 motion = player.getDeltaMovement();
		Direction wallDir = findAvailableWallDirection(player, wallData);
		...
		if (wallDir != null) {
			...
			setCollisionDir(player, wallData, wallDir);
		}
	}

	private static void setCollisionDir(Player player, WallData wallData, Direction direction) {
		// 判断方向是否改变
		if (wallData.getWallSlideCollisionDir() != direction) {
			// 更新数据
			wallData.setWallSlideCollisionDir(direction);

			// 既然服务端也会同步执行，直接由服务端发送广播包给周围的客户端
			if (!player.level().isClientSide && player instanceof ServerPlayer serverPlayer) {
				PacketDistributor.sendToPlayersTrackingEntity(serverPlayer,
					new BroadcastWallSlideDirS2CPayload(serverPlayer.getId(), direction)
				);
			}
		}
	}
	 ...
	 */
	public static Direction findAvailableWallDirection(Player player, WallData wallData) {
		Direction cachedDir = wallData.getWallSlideCollisionDir();
