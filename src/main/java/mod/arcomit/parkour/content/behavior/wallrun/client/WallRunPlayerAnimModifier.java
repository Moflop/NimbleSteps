package mod.arcomit.parkour.content.behavior.wallrun.client;
		...
		if (this.amplitude > 0.001f || this.amplitudeO > 0.001f) {
			...
			switch (boneName) {
				...
				case "body":
					...
					float currentVanillaBodyYaw = Mth.lerp(this.partialTick, this.player.yBodyRotO, this.player.yBodyRot);
					// 2. 获取玩家绝对面朝方向（东南西北）对应的角度
					WallData wallData = ParkourContext.get(player).wallData();
					float targetYaw = Direction.from3DDataValue(wallData.getWallRunMovementDir3DData()).toYRot();
					// 3. 计算需要补偿的差值
