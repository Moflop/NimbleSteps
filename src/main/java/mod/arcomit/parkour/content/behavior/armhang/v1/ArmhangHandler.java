...
//import mod.arcomit.nimblesteps.network.serverbound.armhang.ServerboundSyncArmHangingDirectionPacket;
...
//
//		// 尝试直接支撑上墙
//		boolean canMount = ServerConfig.enableMount &&player.jumping;
//
//		if (canMount) {
...
//		input.leftImpulse = 0; // 防止额外移动
//	}
//
//	private static void startArmhang(MovementStateContext state, Direction direction) {
//		state.setArmHanging(true);
//		state.setArmHangingDirection(direction.get3DDataValue());
//	}
//
//	public static void endArmhang(MovementStateContext state) {
//		state.setArmHanging(false);
//		state.resetArmHangingDirection();
//	}
//
//	private static void applyArmhangMovement(Player player, MovementStateContext state) {
...
//	 */
//	private static void handleInnerCornerRotation(Player player, MovementStateContext state) {
...
//	 * @return 是否成功转角
//	 */
//	private static boolean handleOuterCornerRotation(Player player, MovementStateContext state,
...
//	}
//
//	private static boolean canStartArmhang(Player player, MovementStateContext state) {
...
//	}
//
//	private static boolean canArmhang(Player player, MovementStateContext state) {
...
//
//		// 顶部检测：必须有抓握点，没有方块阻挡
//		AABB topBox = new AABB(minX, baseY, minZ, maxX, baseY + topOffset, maxZ).deflate(0.001);
//		// 底部检测：必须有方块支撑点
//		AABB bottomBox = new AABB(minX, baseY, minZ, maxX, baseY - (height * BOTTOM_COLLISION_MIN_HEIGHT_RATIO), maxZ).deflate(0.001);
//
